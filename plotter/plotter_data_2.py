...
def _read_run(path):
    ...
    label = str(settings.get("Method", os.path.basename(path)))
...
def plot_results_from_excel(
    ...
    # Figure 2: inputs
    # ---------------------------------------------------------
    fig_u, axes = plt.subplots(len(methods), 2,
                               figsize=(12, 2.6 * len(methods)),
                               squeeze=False, sharex=True)
    for i, m in enumerate(methods):
        df = runs[m][1]
        for j, (col, ylab) in enumerate([("input_0", r"$v$ [m/s]"),
                                         ("input_1", r"$\omega$ [rad/s]")]):
            a = axes[i, j]
            u = _column(df, col)
            if u is not None:
                t = np.arange(len(u)) * dt
                ok = np.isfinite(u)
                # mask t, never re-index it: dropping interior NaNs and
                # then rebuilding arange shifts every later sample earlier
                a.plot(t[ok], u[ok], color=color[m], linewidth=1.2,
